...
$OptionSets = "virtual_address" -split " "
...
Foreach (${TestName} in ${ToolsTests} -split " ")
{
	...
	$ResultRun = RunTestBinary ${TestExecutablesDirectory} "wrc_test_tools_${TestName}"
...
For ($ProfileIndex = 0; $ProfileIndex -le ($Profiles.length - 1); $ProfileIndex += 1)
{
	...
	{
		$ResultRun = RunToolsBinaryAndCompareStdout ${TestExecutablesDirectory} "wrcinfo" ${TestProfile} "" ${TestInput}
