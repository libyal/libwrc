...
TEST_EXECUTABLE="../wrctools/wrcinfo";

if ! test -x "${TEST_EXECUTABLE}";
then
	TEST_EXECUTABLE="../wrctools/wrcinfo.exe";
fi
...
TEST_PROFILE_DIRECTORY=$(get_test_profile_directory "input" "wrcinfo");
...
";
	...
	then
		...
		do
			run_test_on_input_file_with_options "${TEST_SET_DIRECTORY}" "wrcinfo" "with_stdout_reference" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "${INPUT_FILE}";
			RESULT=$?;
		...
		do
			run_test_on_input_file_with_options "${TEST_SET_DIRECTORY}" "wrcinfo" "with_stdout_reference" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "${INPUT_FILE}" "${OPTIONS}";
			RESULT=$?;
