...
void (*wrctools_signal_signal_handler)( wrctools_signal_t ) = NULL;

/* Signal handler for Ctrl+C or Ctrl+Break signals
 */
BOOL WINAPI wrctools_signal_handler(
             wrctools_signal_t signal )
{
	static char *function = "wrctools_signal_handler";
	...
	switch( signal )
	{
		...
		case CTRL_C_EVENT:
			if( wrctools_signal_signal_handler != NULL )
			{
				wrctools_signal_signal_handler(
				 signal );
			}
			return( TRUE );
 ...
 */
void wrctools_signal_initialize_memory_debug(
      void )
{
	int flag = 0;
 ...
 */
int wrctools_signal_attach(
     void (*signal_handler)( wrctools_signal_t ),
     libcerror_error_t **error )
{
	static char *function = "wrctools_signal_attach";
	...
	wrctools_signal_signal_handler = signal_handler;

	if( SetConsoleCtrlHandler(
	     (PHANDLER_ROUTINE) wrctools_signal_handler,
	     TRUE ) == 0 )
	{
		libcerror_error_set(
		 error,
...
#if defined( _MSC_VER )
	wrctools_signal_initialize_memory_debug();
#endif
 ...
 * Returns 1 if successful or -1 on error
 */
int wrctools_signal_detach(
     libcerror_error_t **error )
{
	static char *function = "wrctools_signal_detach";
	...
	wrctools_signal_signal_handler = NULL;
